CREATE OR REPLACE PACKAGE PCK_MEDICAL_APPOINTMENT_DETAIL IS
    ...
    PROCEDURE Proc_Insert_MEDICAL_APPOINTENT_DETAIL(
    ...
    PROCEDURE PRoc_Get_MEDICAL_APPOINTENT_DETAIL_BY_ID(
        Ip_detail_id IN NUMBER,
        Op_medical_appointment_detail OUT SYS_REFCURSOR
    );
...
CREATE OR REPLACE PACKAGE BODY PCK_MEDICAL_APPOINTMENT_DETAIL AS
    ...
    END Proc_Insert_MEDICAL_APPOINTENT_DETAIL;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE_APPLICATION_ERROR(-20002, 'An error was encountered -'||SQLCODE||' - Error - ' ||SQLERRM);
    END Proc_Update_MEDICAL_APPOINTENT_DETAIL;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE_APPLICATION_ERROR(-20002, 'An error was encountered -'||SQLCODE||' - Error - ' ||SQLERRM);
    END Proc_Get_All_MEDICAL_APPOINTENT_DETAIL;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE_APPLICATION_ERROR(-20002, 'An error was encountered -'||SQLCODE||' - Error - ' ||SQLERRM);
    END PRoc_Get_MEDICAL_APPOINTENT_DETAIL_BY_ID;
